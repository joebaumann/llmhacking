...
1. Essay tasks (`essay_domestic`, `essay_housewife`, `essay_living`, `essay_location`, `essay_narrative`, `essay_worksocial`):
    ...
    b. To regenerate the preprocessed data from the raw source, run the full preprocessing pipeline: `python -m data.essay.preprocess_data` followed by `python -m data.prepare_final_deduplicated_data` (see [Replicating Original Data Preprocessing](#replicating-original-data-preprocessing)). If you have obtained legal access to the underlying data and would like the exact preprocessed files, please contact us ([joachimbaumann1@gmail.com](mailto:joachimbaumann1@gmail.com)).
2. Issue_survey task:
